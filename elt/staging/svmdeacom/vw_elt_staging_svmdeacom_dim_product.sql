with dmprod as (
    select 
        ...
        ,   _rivery_run_id
        ,   _rivery_last_update
    from {{ref('vw_stg_svmdeacom_dmprod')}} 
),
dmcats as (
	select ca_id,ca_name
	from {{ref('vw_stg_svmdeacom_dmcats')}}
),
dmcats2 as (
	select c2_id,c2_name
	from {{ref('vw_stg_svmdeacom_dmcats2')}}
),
dmpr1 as (
	select p1_id,p1_name
	from {{ref('vw_stg_svmdeacom_dmpr1')}}
),
dmunit as (
	select un_id, un_name 
	from {{ref('vw_stg_svmdeacom_dmunit')}}
)
select      
